	...
	<div class="container">
		<form action="subList.do?uri=main" method="post">
			<table border='1'>
				...
				<tr>
					<td colspan="22" height="50px">
						<input type="text" name="search" style="width: 40%; height: 24px; margin: 30px 0;" value="" />
						<input class="input_btn" type="submit" value="検索" style="width: 70px; height: 30px;" />
					</td>
				</tr>
				<tr id="tr_title">
				...
				<c:forEach var="sub" items="${sub }">
					...
					<td>${sub.lease }</td> --%>
						<td>
							<input class="submit_btn" style="width: 40px; height: 25px; font-size: 10px;" type="button" value="確認"
								onclick="location.href = 'receipt.do?no=${sub.no }'" />
						</td>
					</tr>
				</c:forEach>
				<tr>
					<td id="tr_page_a" colspan="22">
						<c:if test="${paging.total/paging.limit<=5 }">
							<c:forEach var="currentPage" begin="${paging.startPage }" end="${paging.total/paging.limit}">
								<a class="page_a" href="subList.do?uri=main&currentPage=${ currentPage}&limit=${paging.limit }">${currentPage }</a>
							</c:forEach>
						</c:if>
						<c:if test="${paging.endPage>5 }">
							<a class="page_a" href="subList.do?uri=main&startPage=${ paging.startPage-5}&limit=${paging.limit }&currentPage=${ paging.startPage -1}">
								前へ</a>
						</c:if>
						<c:if test="${paging.total/paging.limit>5 }">
							<c:forEach var="currentPage" begin="${paging.startPage }" end="${ paging.startPage +4 }">
								<a class="page_a" href="subList.do?uri=main&currentPage=${ currentPage}&limit=${paging.limit }&startPage=${ paging.startPage}">${currentPage }</a>
							</c:forEach>
							<a class="page_a" href="subList.do?uri=main&startPage=${ paging.endPage+1}&limit=${paging.limit }&currentPage=${ paging.endPage +1}">次へ</a>
						</c:if>
					</td>
				</tr>
			</table>
		...
		<div id="btn_div">
			<button class="submit_btn" type="button" onclick="location.href='subList.do'" style="width: 150px;">水道代、電気代情報</button>
			<button class="submit_btn" type="button" onclick="location.href='<%=request.getContextPath()%>/unit/list.do'">戻る</button>
		</div>
	</div>
</body>
</html>